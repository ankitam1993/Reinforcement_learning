...
int QLambda::getAction(const Eigen::VectorXd &features, std::mt19937_64 &generator) {
    ...
    if (d(generator)) // This returns true with probability epsilon, due to the way that d was initialized in the constructor (up top)
        return explorationDistribution(
                generator);    // Explore by selecting an action uniformly randomly (this is what the explorationDistribution was initialized to do)
    ...
    vector<int> bestActions(
            1);                    // Here we will track which actions are currently believed to be optimal
    ...
    for (int a = 1; a < numActions; a++)        // Loop over other actions and see if they are better
    {
        if (qValues[a] == bestActionValue)
            bestActions.push_back(
                    a);            // Action a is just as good as the current actions believed to be best. Append to bestActions
...
void QLambda::train(const VectorXd &features, const int &action, const double &reward, const VectorXd &newFeatures) {
    ...
    curQ = features.dot(w.segment(this->numFeatures * action, this->numFeatures));
    // Compute max_a', q(s',a'), where s' is described by newFeatures
    double maxQnext =  newFeatures.dot(w.segment(0, numFeatures));
    for (int a = 1; a < numActions; a++)        // Loop over other actions and see if they are better
    {
        double Qnext = newFeatures.dot(w.segment(numFeatures * a, numFeatures));
        if (Qnext > maxQnext) {
            maxQnext = Qnext;
        }
    }
    ...
    delta = reward + this->gamma * newQ - curQ;
    ...
    e =gamma * lambda * e;
    e.segment(numFeatures * action, numFeatures) += features;
