...
ActorCritic::ActorCritic(int numFeatures, int numActions, double alpha_actor, double alpha_critic, double gamma,
                         double lambda) {
...
void
ActorCritic::train(const VectorXd &features, const int &action, const double &reward, const VectorXd &newFeatures) {
    ...
    theta += alpha_actor * delta * eTheta;
}
...
VectorXd ActorCritic::dlnpi(const Eigen::VectorXd &features, const int &action) {
    ...
    for (int a = 0; a < numActions; a++) {
        if (a == action)
            result.segment(numFeatures * a, numFeatures) = (1 - actionProbabilities[action]) * features;
        else
            result.segment(numFeatures * a, numFeatures) = -(actionProbabilities[a]) * features;
    }
    return result;
}
