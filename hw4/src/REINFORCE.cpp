...
void REINFORCE::train(const VectorXd &features, const int &action, const double &reward) {
    ...
    returns[len - 1] = rewardHistory[len - 1];
    for (int a = len - 2; a >= 0; a--) {
        returns[a] = gamma * returns[a + 1] + rewardHistory[a];
    }
    ...
    for (int a =0;a < len;a++){
        theta = theta + alpha_actor*errors[a]*dlnpi(featureHistory[a],actionHistory[a]);
    }
    // Update baseline
...
VectorXd REINFORCE::dlnpi(const Eigen::VectorXd &features, const int &action) {
    ...
    for (int a = 0; a < numActions; a++) {
        if (a == action)
            result.segment(numFeatures * a, numFeatures) = (1 - actionProbabilities[a]) * features;
        else
            result.segment(numFeatures * a, numFeatures) = (-1 * actionProbabilities[a]) * features;
    }
    return result;
