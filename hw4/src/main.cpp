...
void run() {
    string agentName = "ActorCritic";
    cout << "Grid search for " + agentName << endl;
    int numTrials = 2;
    int numEpisodes = 200;
    int order = 0;
    ...
    for (double alpha_actor = 0.1; alpha_actor <= 1.0; alpha_actor += 0.1) {
        for (double alpha_critic = 0.1; alpha_critic <= 1.0; alpha_critic += 0.1) {
            for (double lambda = 0.1; lambda <= 1.0; lambda += 0.1) {
                for (double gamma = 0.1; gamma <= 1.0; gamma += 0.1) {
                    cout << "Inputs" << endl;
                    cout << "Alpha Actor: " + to_string(alpha_actor) << endl;
                    cout << "Alpha Critic: " + to_string(alpha_critic) << endl;
                    cout << "Lambda: " + to_string(lambda) << endl;
                    cout << "Gamma: " + to_string(gamma) <<endl;
                    for (int trialCount = 0; trialCount < numTrials; trialCount++) {
                        Environment *e = new Gridworld();
                        int envType = 0;
                        FourierBasis fb(e->getStateDim(), e->getMinState(), e->getMaxState(), order, order,
                                        envType == 0);
                        ActorCritic *a = new ActorCritic(fb.getNumOutputs(), e->getNumActions(), alpha_actor,
                                                         alpha_critic, gamma, lambda);
                        vector<VectorXd> results(numTrials);
                        results[trialCount] = runAgentEnvironment(a, e, &fb, numEpisodes, generators[trialCount]);
                    }
                    for (int epCount = numEpisodes - 1; epCount < numEpisodes; epCount++) {
                        ...
                        cout << "Final Return: " + to_string(v.mean()) <<endl;
                        //cout << v.mean();
                    }
                }
