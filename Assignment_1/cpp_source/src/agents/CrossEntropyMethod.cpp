...
static void CEMUpdate(
	...
	const int & episodesPerPolicy)
{
	// @TODO: For HW1, you should fill in this function. The two lines below
	// are just a place-holder.

    /*
	VectorXd temp;
	MatrixXd sigma_temp = MatrixXd::Zero(numParams, numParams);
	theta.setZero();
    ...
    for(int i = 0 ; i < numElite ; i++)
    {
        //temp = (thetas[i] - theta) * ((thetas[i] - theta).transpose());
        temp = thetas[i] - theta;
        sigma_temp += temp * (temp.transpose());

        //Sigma += temp;
    }

    Sigma = (Sigma*100 + sigma_temp ) / (numElite + 100);

    //Sigma = Sigma / numElite; */

    if (numElite > thetas.size()) {
		cout << "Elite population is too big";
		theta.setZero();
		Sigma = MatrixXd::Identity(numParams, numParams);
		return;
	}
	...
	VectorXd newTheta(theta.size());
	newTheta.setZero();
	for (int i = 0; i < numElite; i++) {
		newTheta = newTheta + thetas[i];
	}
	newTheta /= numElite;

	// Computing the covariance
	MatrixXd newSigma = Sigma;
	newSigma.setZero();
	for (int i = 0; i < numElite; i++) {
		VectorXd delta = thetas[i] - newTheta;
		MatrixXd cov = delta * delta.transpose();
		newSigma = newSigma + cov;
	}
	MatrixXd identity = Sigma;
	identity.setIdentity();
	newSigma = newSigma + epsilon*identity;
	newSigma = newSigma/(numElite + epsilon);
	theta = newTheta;
	Sigma = newSigma;
}
